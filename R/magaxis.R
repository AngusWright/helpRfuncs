magaxis <- function (side = 1:2, majorn = 5, minorn = "auto", tcl = 0.5, 
    ...
    grid.col = "grey", grid.lty = 1, grid.lwd = 1, axis.lwd = 1, 
    ...
    usemultloc, ...) 
{
    ...
    for (i in 1:length(side)) {
        ...
        if (unlog) {
            sci.tick = magicaxis::maglab(10^lims, n = majorn, log = T, exptext = T, 
        ...
        if (logged & unlog == F) {
            sci.tick = magicaxis::maglab(10^lims, n = majorn, log = T, exptext = F, 
                crunch = crunch, logpretty = logpretty, usemultloc = usemultloc, 
                prettybase = prettybase, powbase = powbase, hersh = hersh)
            major.ticks = log(sci.tick$tickat, powbase)
            uselabels = sci.tick$exp
        ...
        if (logged == F & unlog == F) {
            sci.tick = magicaxis::maglab(lims, n = majorn, log = F, exptext = F, 
        ...
        if (logged) {
            do.call("axis", c(list(side = currentside, at = powbase^major.ticks, 
                tcl = tcl, labels = FALSE, tick = do.tick, mgp = mgp, 
                lwd = axis.lwd, lwd.ticks = ticks.lwd, col = axis.col), 
                dotsaxis))
        }
        else {
        ...
        if (labels) {
            if (logged) {
                do.call("axis", c(list(side = currentside, at = powbase^labloc, 
                  tick = F, labels = uselabels, mgp = mgp, lwd = axis.lwd, 
                  lwd.ticks = ticks.lwd, col = axis.col), dotsaxis))
            }
            else {
        ...
        if (usemultloc == F & minorn > 1) {
            ...
            if (logged) {
                do.call("axis", c(list(side = currentside, at = powbase^minor.ticks, 
                  tcl = tcl * ratio, labels = FALSE, tick = do.tick, 
                  mgp = mgp, lwd = axis.lwd, lwd.ticks = ticks.lwd, 
                  col = axis.col), dotsaxis))
            }
            else {
